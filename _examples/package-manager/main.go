...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case installedPkgMsg:
		...
		glyph := GlyphForPackage(m.packages[m.index])

		return m, tea.Batch(
			progressCmd,
			tea.Printf("%s %s %s", checkMark, glyph, m.packages[m.index]), // print success message above our program
...
func (m model) View() tea.View {
	...
	if m.done {
		checkMark := lipgloss.NewStyle().Foreground(theme.Green).SetString("✓")
		return tea.NewView(doneStyle.Render(fmt.Sprintf("%s Done! Installed %d packages.\n", checkMark, n)))
	}
	...
	info := lipgloss.NewStyle().MaxWidth(cellsAvail).Render("Installing " + glyph + " " + pkgName)
